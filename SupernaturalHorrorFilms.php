...
SELECT ?Film_name ?film_wiki_link ?name_of_director 
?director_wiki_link ?name_of_star ?star_wiki_link
WHERE {
  ?film dcterms:subject category:Supernatural_horror_films ;
              dbprop:name ?Film_name ;
			 dbpedia-owl:director ?director;
			 dbpedia-owl:starring ?stars;
           foaf:isPrimaryTopicOf ?film_wiki_link .
  ?director foaf:name ?name_of_director .
  ?stars foaf:name ?name_of_star .
  OPTIONAL {
    ?director foaf:isPrimaryTopicOf ?director_wiki_link .
}
  OPTIONAL {
    ?stars  foaf:isPrimaryTopicOf ?star_wiki_link .
  }
...
foreach($results as $res) {
 if(!in_array($res->Film_name->value."-".$res->film_wiki_link->value,$filmArr)){
	if($j!=0)
	$i++;
	$filmArr[$i] = $res->Film_name->value."-".$res->film_wiki_link->value;
