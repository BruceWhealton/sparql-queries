  ...
  $(document).ready(function(){
    ...
    PREFIX dbcat: <http://dbpedia.org/resource/Category:>\
    PREFIX skos: <http://www.w3.org/2004/02/skos/core#> \
    SELECT DISTINCT ?an ?bn ?cn ?abstract ?wiki_link \
    WHERE \
  ...
  var parseLangResults = function(data){
     console.log(data);
	  ...
	  detail = data.results.bindings[0];
     ...
     // $("#lang-result-details").empty();
      // fill in
	     ...
	     domEntry += detail.wiki_link.value;
		 domEntry += '">';
		 domEntry += detail.an.value;
		 domEntry += '</a>';
		 domEntry += '</h1>';
		 domEntry += '<p>' + detail.abstract.value + '</p>';
		 domEntry += '<p>';
		 for(i = 0; i < len; i++){
		     detail = data.results.bindings[i];
			   domEntry += detail.an.value;
			   domEntry += ' was influenced by the language ';
			   domEntry += detail.bn.value;
			   domEntry += '</p>';
			   domEntry += '<p>';
			   domEntry += detail.an.value;
			   domEntry += ' influenced the development of the language ';
			   domEntry += detail.cn.value;
			   domEntry += '</p>';  
			   domEntry += '<h3>For more information about this language, ';
			   domEntry += detail.an.value;
			   domEntry += ' click on the link above.</h3> ';
